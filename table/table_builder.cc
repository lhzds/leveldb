...
struct TableBuilder::Rep {
  Rep(const Options& opt, WritableFile* f, uint64_t fn, pmem_index::PMIndex* pmidx)
      : options(opt),
...
TableBuilder::TableBuilder(const Options& options, size_t file_number, WritableFile* file, pmem_index::PMIndex* pm_index)
    : rep_(new Rep(options, file, file_number, pm_index)) {
...
void TableBuilder::WriteBlock(BlockBuilder* block, BlockHandle* handle, bool writeToBuffer) {
...
void TableBuilder::WriteRawBlock(const Slice& block_contents,
  ...
  if (r->status.ok()) {
    ...
    SetBlockTrailer(trailer, block_contents, type);
...
void TableBuilder::SetBlockTrailer(char* trailer, const Slice& block_contents, CompressionType type) {
  trailer[0] = type;
...
Status TableBuilder::Finish() {
  ...
  if (ok()) {
    ...
    WriteBlock(&meta_index_block, &metaindex_block_handle, true);
  }
  ...
  if (ok()) {
    ...
    WriteBlock(&r->index_block, &index_block_handle, true);
  }
